...
* PostalCode (string) … 申込時点の送付先郵便番号
* Address1 (string) … 申込時点の送付先住所1
* Address2 (string) … 申込時点の送付先住所2
* Address3 (string?) … 申込時点の送付先住所3
* PhoneNumber (string?) … 申込時点の送付先電話番号
* TotalPoints (int)
* OrderedAt (DateTime)
* IsCancelled (bool)
...
※ 株主マスタ（M_Shareholder）の住所・電話番号は基本情報として保持し、申込時に入力・確認した送付先は注文時点のスナップショットとして T_RewardOrder に保存する。
...
* 送付先住所・電話番号は T_RewardOrder に保存された申込時点の値を出力する。
...
4. POST /api/shareholder/order
   ...
   * 入力：送付先情報（PostalCode, Address1, Address2, Address3, PhoneNumber）と ItemId & Quantity のリスト
   * バリデーション：
   ...
   * 保存：
     ...
     * 申込時に確認した送付先情報を T_RewardOrder に保存する。
     * M_Shareholder の住所・電話番号は申込時には更新しない。
...
* /confirm
  → 申込内容確認・同意チェック。
  送付先住所・電話番号を確認し、この画面で確定した値を申込時に T_RewardOrder へ保存する。
...
* 申込登録時：
  ...
  * 確認画面で入力した送付先住所・電話番号を T_RewardOrder に設定する。
  * 合計ポイント ≤ 使用可能ポイント の場合のみ T_RewardOrder / T_RewardOrderItem を登録。
  * M_Shareholder の住所・電話番号は変更しない。
